...
Usage (from project root):
  PowerShell -ExecutionPolicy Bypass -File .\scripts\run_all_fixed.ps1
#>
...
try {
  ...
  Start-Process -FilePath cmd.exe -ArgumentList '/k','cd /d "' + $backendDir + '" && npm run start' -WorkingDirectory $backendDir -WindowStyle Normal

  Start-Sleep -Seconds 1

  Write-Output "Starting frontend preview (npm run preview) in a new terminal..."
  Start-Process -FilePath cmd.exe -ArgumentList '/k','cd /d "' + $root + '" && npm run preview' -WorkingDirectory $root -WindowStyle Normal
